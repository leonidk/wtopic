...
for vs in ['vec','idf']:
	if vs == 'vec':
		...
		vectorizer = TfidfVectorizer(dtype=np.int32,stop_words='english',max_df=0.8,min_df=1e-3,strip_accents='ascii') #TfidfVectorizer #CountVectorizer
	...
	# fit topic model
	models = ['LSA','wLSA','wLSA2','LDA','wLDA','wLDA2']
	...
	for model in models:
		for k in [5,10,20,30]:
			if model == 'LDA':
				...
				u,s,vt = svds(X.todense().astype(np.float32).dot(probs),k)
				res = vt
				doc = u
			elif model == 'wLSA2':
				u,s,vt = svds(X.todense().astype(np.float32).dot(probs.T),k)
				res = vt
				doc = u
			else:
			...
			for metric in ['cosine','euclidean']:
				...
				print("{0}\t{1}\t{2}\t{3}\t{4:.3f}".format(metric[:6],model,k,vs,correct/total))
